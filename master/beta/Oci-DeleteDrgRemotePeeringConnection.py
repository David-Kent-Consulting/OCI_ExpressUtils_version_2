...
from lib.gateways import delete_drg_rpc
...
if len(sys.argv) < 6 or len(sys.argv) > 7:
    print(
        ...
        "Add the --force option to not be prompted prior to deleting the remote peering connection.\n\n"
...
# check to see if the RPC exists, if not, raise an alarm
...
if len(sys.argv) == 6:
    warning_beep(6)
    print("Enter YES to proceed with deleting the remote peering connection {} from Dynamic router gateway {}".format(
        drg_rpc_name,
        dynamic_router_gateway_name
    ))
    if "YES" != input():
        print ("Delete request of remote peering connection aborted per user request.\n\n")
        exit(0)
elif option != "--FORCE":
    warning_beep(1)
    raise RuntimeWarning("INVALID OPTION! - The only valid option is --force")

# delete the RPC resource
print("\nDeleting the remote peering connection resource. This can take up to 5 minutes to complete......\n")
delete_rcp_response = delete_drg_rpc(
    network_composite_client,
    drg_rpc.id
)

if delete_rcp_response is None:
    raise RuntimeError("EXCEPTION! - UNKNOWN ERROR")
else:
    print("\n\nDeletion of remote peering gateway {} from dynamic router gateway {} was successful.\n\n".format(
        drg_rpc_name,
        dynamic_router_gateway_name
    ))
