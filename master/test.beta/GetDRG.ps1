...
{
    $count = $myDRGs.data.Count
    $cntr  = 0
    while ( $cntr -le $count ) {
        if ( $myDRGs.data[$cntr].'display-name' -eq $myDrgName -and $myDRGs.data[$cntr].'lifecycle-state' -eq 'AVAILABLE' ) {
            return $myDRGs.data[$cntr]
        }
    }
}
...
$DRGs           = oci network drg list --compartment-id $myCompartment.id | ConvertFrom-JSON
if (!$DRGs) {
  Write-Output "No IGWs found in compartment $ComparmentName. Please try again."
  return 1}

$return         = GetDRG $DRGs $DrgName

if (!$return){
  Write-Output "DRG name $DrgName not found in compartment name $CompartmentName for VCN name $VcnName. Please try again."
  return 1
} else {
  ReturnValWithOptions "GetNGW.ps1" $return $options
}
