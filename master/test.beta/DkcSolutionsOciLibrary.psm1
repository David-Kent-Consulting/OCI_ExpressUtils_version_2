...
{
    $count = $myFileSystems.data.count
    $cntr  = 0
    while ( $cntr -le $count ){
        if ( $myFileSystems.data[$cntr].'display-name' -eq $myFileSystemName ) {return($myFileSystems.data[$cntr])}
        $cntr=$cntr+1
    }
    if ( $myFileSystems.data.'display-name' -eq $myFileSystemName ) {return($myFileSystems.data)}
...
function GetMountTarget(
  ...
  [string]$myMountTargetName
    ...
    $cntr  = 0
    $count = $myMountTargets.data.count
    while ($cntr -le $count){
      if ($myMountTargets.data[$cntr].'display-name' -eq $myMountTargetName -and $myMountTargets.data[$cntr].'lifecycle-state' -eq "ACTIVE") {
        return($myMountTargets.data[$cntr])
        }
      $cntr=$cntr+1
    }
    if ($myMountTargets.data.'display-name' -eq $myMountTargetName -and $myMountTargets.data.'lifecycle-state' -eq "ACTIVE"){
      return($myMountTargets.data)
    }
