...
if len(sys.argv) != 5:
    print(
        ...
        "Oci-AddLinuxVmsFromCsvFile.py [parent compartment] [child compartment] [region] [csv import file]\n\n" +
        "Use case example adds VM instances to the specified child compartment within the specified region:\n" +
        "\tOci-AddLinuxVmsFromCsvFile.py admin_comp web_comp 'us-phoenix-1' drtest_vms_to_build.csv\n\n" +
...
def return_host_record(cntr):
    ...
    if check_for_vm(
        ...
        child_compartment.id,
        vm_name):
        print("VM {} already present in compartment {} and is being skipped. Duplicate VMs not permitted.\n".format(
            vm_name,
...
while cntr < count:
    ...
    if my_host is None:
        # do nothing if host already exists
        pass
    else:
