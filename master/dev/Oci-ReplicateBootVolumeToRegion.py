...
if len(sys.argv) != 6:
    print(
        "\n\nOci-ReplicateBootVolumeToRegion.py\n" +
        "Oci-ReplicateBootVolumeToRegion.py [parent compartment] [child compartment] [backup item ID] [source region] [destination region]\n\n" +
        "Use case example replicates the specified boot volume item to the specified destination region:\n" +
        "\tOci-ReplicateBootVolumeToRegion.py admin_comp dbs_comp \\\n" +
        "\t'ocid1.bootvolumebackup.oc1.iad.abuwcljtslmyxmeakhrxbwwolo7b5vdi4e4jn5qwasti6atfhrzug4fl57wq' \\\n" +
...
# make sure the item type is a boot vol backup
if "bootvolumebackup" not in backup_item_id:
    raise RuntimeWarning("WARNING! - Backup item must be of type boot volume type")
...
backup_item = storage_client.get_boot_volume_backup(
    boot_volume_backup_id = backup_item_id
).data

print("Submitting boot volume backup replication request for item {} with the name {} in compartment {} from region {} to region {}\n\n".format(
    backup_item_id,
...
replicate_backup_item_request_response = storage_client.copy_boot_volume_backup(
    boot_volume_backup_id = backup_item.id,
    copy_boot_volume_backup_details = CopyVolumeBackupDetails(
