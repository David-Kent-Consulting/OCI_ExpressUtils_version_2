...
switch ($options){
    ...
    "EMAIL" {return($result.email)}
    "STATUS" {return($result.'lifecycle-state')}
    "CAPABILITIES" {return($result.capabilities)}
    Default {
        Write-Output "Incorrect option. Correct options are [ALL] [OCID] [EMAIL] [STATUS] [CAPABILITIES]"
