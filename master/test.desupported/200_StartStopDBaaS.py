...
DB_STATE                    = sys.argv[2]
DB_REGION                   = str.lower(sys.argv[3])
...
def ChangeDbNodeState(my_DB_NODE_ID, my_DB_STATE):
    if str.upper(my_DB_STATE) == 'START':
        ...
        cmd = ('oci db node start --db-node-id '+my_DB_NODE_ID+' --region '+DB_REGION)
        ...
        cmd = ('oci db node stop --db-node-id '+my_DB_NODE_ID+' --region '+DB_REGION)
        sp  = subprocess.Popen([cmd], shell=True, stdout=subprocess.PIPE, stderr=None)
        output, _ = sp.communicate()
        results   = output.decode("utf-8")
        print(results)
...
try:
    ...
    print("\n\nThe job request for changing the virtual machine database state has been completed.\n")
