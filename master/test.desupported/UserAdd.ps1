...
$Users = GetUsers $TenantObjects.TenantId.data.id

$result = GetUser $Users $UserName
if ($result){
    Write-Output "User $UserName already present."
    return $false
}

$Groups = GetGroups $TenantObjects.TenantId.data.id
$Group  = GetGroup $Groups $GroupName
if (!$Group){
    Write-Output "Group $GroupName not found. Please try again with a valid group name."
    return $false
}


python useradd.py $UserName $email $description $Group.id