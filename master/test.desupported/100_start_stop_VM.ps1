...
switch ($options){
  "START" {
    if ($VM.'lifecycle-state' -eq 'RUNNING' -or $VM.'lifecycle-state' -eq 'STARTING') {
      Write-Output "VM $VmName presenting in a running or starting state. - NO ACTION TAKEN"
      return
    }
    python 100_start_stop_VM.py $VM.id $Region "START"
  }
  "STOP" {
    if ($VM.'lifecycle-state' -eq 'STOPPED' -or $VM.'lifecycle-state' -eq 'STOPPING') {
      Write-Output "VM $VmName presenting in a stopped or stopping state. - NO ACTION TAKEN"
      return
    }
    python 100_start_stop_VM.py $VM.id $Region "STOP"
  }
  Default {
    Write-Output "Invalid option, correct options are [START] or [STOP]."
