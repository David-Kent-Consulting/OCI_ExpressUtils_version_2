...
$FileSystems    = GetFileSystems $myCompartment $AvailabilityDomain $Region
if (!$FileSystems) {
    Write-Output "No file systems present in compartment $CompartmentName for availability domain $AvailabilityDomain."
    Write-Output "Please try again."
    return $false
}

$result         = GetFileSystem $FileSystems $FileSystemName

switch($options){
  "ALL"         {return($result)}
  "OCID"        {return($result.id)}
  Default       {
                  Write-Output "Invalid option, options are [ALL] [OCID]"
                  return $false
                }
}
