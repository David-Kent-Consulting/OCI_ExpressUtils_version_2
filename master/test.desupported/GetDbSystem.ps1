...
switch ($options) {
    "ALL"       {return ($return)}
    "OCID"      {return ($return.id)}
    "SHAPE"     {return ($return.shape)}
    "STATUS"    {return ($return.'lifecycle-state')}
    "EDITION"   {return($return.'database-edition')}
    "HOSTNAME"  {return($return.hostname)}
    "LISTENER"  {return($return.'listener-port')}
    "NODECOUNT" {return($return.'node-count')}
    "SSHKEYS"   {return($return.'ssh-public-keys')}
    default {
        Write-Output "Invalid option. Valid options are:"
        Write-Output "ALL - All Db System properties"
        Write-Output "OCID - The OCID of the DB system"
        Write-Output "SHAPE - The virtual machine size, or shape, that the DB System is set to"
        Write-Output "EDITION - The edition of the DB System"
        Write-Output "HOSTNAME - The name of the DB System service node(s)"
        Write-Output "LISTENER - The TCP port the DB System is set to listen on"
        Write-Output "NODECOUNT - The number of the service nodes associated with the DB System"
        Write-Output "SSHKEYS - The SSH keys that are applied to the DB System for SSH key authentication"
        Write-Output " "
        Write-Output "Please try again."
        return 1
    }
