...
$IGWs           = oci network internet-gateway list --compartment-id $myCompartment.id --vcn-id $myVcn.data[0].id | ConvertFrom-JSON
if (!$IGWs) {
  Write-Output "No IGWs found in compartment $ComparmentName. Please try again."
  return 1}

$return         = SelectIGW $IgwName $IGWs

if (!$return){
  Write-Output "NAT Gateway name $NgwName not found in compartment name $CompartmentName for VCN name $VcnName. Please try again."
  return 1
} else {
