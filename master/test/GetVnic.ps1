...
$myVnic         = GetVnic $myVm $Region

if (!$myVnic) {
    Write-Output "WARNING! No virtual network interfaces found for VM $VmName."
    Write-Output "Please investigate or open a service request with Oracle at"
    Write-Output "https://support.oracle.com"
    return 1
}

switch ($options) {
    "ALL"           {return ($myVnic.data)}
    "OCID"          {return ($myVnic.data.'vnic-id')}
    "ATTACHMENTID"  {return ($myVnic.data.id)}

    default {
        Write-Host "Valid options are [ALL] [OCID] [ATTACHMENTID]"
